...
describe('Integration Tests - Console Export', () => {
  ...
  beforeEach(() => {
    ...
    const consoleMethods = ['log', 'dir', 'info', 'warn', 'error', 'debug', 'table'];
    consoleMethods.forEach(method => {
      capturedOutput[method] = [];
      consoleSpies[method] = jest.spyOn(console, method as any).mockImplementation((...args) => {
  ...
  describe('Trace Publishing', () => {
    test('should publish spans to console', async () => {
      ...
      const spanObjects = allSpanOutput.filter(item =>
    ...
    test('should publish nested spans to console', async () => {
      ...
      expect(allNestedOutput.some(item =>
        ...
        (typeof item === 'object' && item && item.name === 'child-operation')
      )).toBe(true);
    });
  });
  ...
  describe('Logs Publishing', () => {
    test('should publish log records to console', async () => {
      ...
      const logObjects = allLogOutput.filter(item =>
  ...
  describe('Combined Telemetry Publishing', () => {
    test('should publish traces, metrics, and logs simultaneously', async () => {
      ...
      expect(allCombinedOutput.some(item =>
        typeof item === 'object' && item && item.descriptor && item.descriptor.name === 'combined_test_counter'
      )).toBe(true);
      expect(allCombinedOutput.some(item =>
        (typeof item === 'string' && item.includes('Combined test log')) ||
        (typeof item === 'object' && item && item.body === 'Combined test log')
      )).toBe(true);
    }, 15000);
  });
});